#!/bin/bash
# 尝试启动 vLLM 0.23.0 服务并测试 NVFP4 KV cache。
# 注意：截至 vLLM 0.23.0，--kv-cache-dtype nvfp4 要求 sm100f，RTX 5090 (sm120) 会报错：
#   "--kv-cache-dtype nvfp4 requires sm100f, please try a different dtype or remove"
# 因此当前在 RTX 5090 上不可用，仅作为实验脚本保留。
...
echo "[INFO] 启动 vLLM 0.23.0 服务（NVFP4 KV cache）"
echo "[INFO] 模型: $MODEL_PATH"
echo "[INFO] 监听: $VLLM_HOST:$VLLM_PORT"
echo "[WARN] RTX 5090 (sm120) 当前不支持 nvfp4 KV cache，仅供测试"
echo ""
