...
async def download_and_preview(username, api_key, tags, exclude_tags, score_threshold, limit, save_path, proxy,
    ...
    if images:
        ...
        return images, tags_list, f"实际下载 {len(images)-deleted_images_count} 张图片，{total_failed_count} 张图片下载失败，初始损坏图片 {initial_invalid_image_count} 张，最终损坏图片 {final_invalid_image_count} 张，重新下载了 {re_downloaded_count} 张图片，删除图片重复{deleted_images_count}张。"
    else:
